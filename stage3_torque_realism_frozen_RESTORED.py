...
# Now we need to write the payload(with the added weight)
class PayloadManager:
    """
    Sabotage as a *parameter change* keyed on env_ids
    """
    def __init__(self, robot: Articulation):
        self.view = robot.root_physx_view
        self.hand_idx = robot.find_bodies("panda_hand")[0][0]
        # Snapshot the factory masses ONCE, before any sabotage ever happens.
        # get_masses() returns shape (num_envs, max_links) -- rows are robots,
        # now we are creating a some sort of copy
        self.default = self.view.get_masses().clone()
    
    def _set(self, env_ids, new_hand_mass):
        m = self.view.get_masses().clone()
        m[env_ids, self.hand_idx] = new_hand_mass
        self.view.set_masses(m, torch.arange(m.shape[0]))
    """New handmass is the factory mass of those envs hands with extra"""
    def apply(self, env_ids, extra):
        # 
        self._set(env_ids, self.default[env_ids, self.hand_idx] + extra)
    """# This restores the factory mass of those envs hands"""
    def clear(self, env_ids):
        self._set(env_ids,self.default[env_ids, self.hand_idx])
...
class BatchedPhysicsMPPI:
    ...
    def __init__(self, robot: Articulation, sim, scene, Kp, Kd, ee_idx,
        ...
        # This is the additional change we are adding
        # A terminal velocity penalty
        self.w_vterm = 10.0
...
if __name__ == "__main__":
    ...
    log_filepath = os.path.join(log_dir, f"v2_execution_{timestamp}.log")
